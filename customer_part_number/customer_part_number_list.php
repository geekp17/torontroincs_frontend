  ...
  <body class="bg-light">
    ...
    <div class="container my-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Customer Part Number List</h1>
        <a class="btn btn-success" href="./customer_part_number_form.php">New Customer Part Number</a>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const RESOURCE = 'customer-part-numbers';
      ...
      listEl.innerHTML = items.map(row => (
        ...
        `<td class="text-end">
           <div class="btn-group btn-group-sm">
             <a class="btn btn-outline-primary" href="./customer_part_number_detail.php?id=${row.id || row.pk}">View</a>
  ...
  document.getElementById('table').addEventListener('click', async (e) => {
    ...
    if (action === 'delete' && confirm('Delete this item?')) {
      const res = await fetch(`${API_BASE}/customer-part-numbers/${id}/`, {
        method: 'DELETE',
        headers: authHeaders()
      });
    ...
    if (action === 'edit') {
      location.href = './customer_part_number_form.php?id=' + id;
    }
  });
  ...
  createForm?.addEventListener('submit', async (e) => {
    ...
    const res = await fetch(`${API_BASE}/customer-part-numbers/`, {
      method: 'POST',
      headers: authHeaders(),
