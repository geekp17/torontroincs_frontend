  ...
  <body class="bg-light">
    ...
    <div class="container my-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Work Order List</h1>
        <a class="btn btn-success" href="./work_order_form.php">New Work Order</a>
      </div>
  ...
  <div class="card-body">
    <form id="createForm" class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input class="form-control" id="create_project" placeholder="project">
      </div>
      <div class="col-12">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadList() {
    ...
    try {
      ...
      listEl.innerHTML = items.map(row => (
        `<tr>`+`<td>${row['name']}</td>`+`<td>${row['project'].name}</td>`+
        `<td class="text-end">
           <div class="btn-group btn-group-sm">
             <a class="btn btn-outline-primary" href="./work_order_detail.php?id=${row.id || row.pk}">View</a>
  ...
  createForm?.addEventListener('submit', async (e) => {
    ...
    payload['project'] = document.getElementById('create_project').value;
