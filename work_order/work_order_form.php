...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h1 class="h3" id="pageTitle">Work Order Form</h1>
  ...
  <div class="card-body">
    <form id="form" class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input class="form-control" id="f_project" placeholder="project">
      </div>
      <div class="col-12">
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (id) {
    ...
    if (res.ok) {
      ...
      var el_project = document.getElementById('f_project'); if (el_project) el_project.value = (data['project'] ?? '');
    }
  } else {
    title.textContent = 'Create Work Order';
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const url = id ? `${API_BASE}/work-orders/${id}/` : `${API_BASE}/work-orders/`;
    const method = id ? 'PATCH' : 'POST';
    const res = await fetch(url, { method, headers: authHeaders(), body: JSON.stringify(payload) });
    if (res.ok) {
      location.href = './work_order_list.php';
    } else {
