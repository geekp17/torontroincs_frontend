  ...
  <body class="bg-light">
    ...
    <div class="container my-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Part Number List</h1>
        <a class="btn btn-success" href="./part_number_form.php">New Part Number</a>
      </div>
  ...
  <div class="card-body">
    <form id="createForm" class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input class="form-control" id="create_details" placeholder="details">
      </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover mb-0" id="table">
        <thead class="table-light">
          <tr>
            <th>Name</th><th>Details</th><th>Customer Part Number</th>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadList() {
    ...
    try {
      ...
      listEl.innerHTML = items.map(row => (
        `<tr>`+`<td>${row['name']}</td>`+`<td>${row['details']}</td>`+`<td>${row['customer_part_number'].name}</td>`+
        `<td class="text-end">
           <div class="btn-group btn-group-sm">
             <a class="btn btn-outline-primary" href="./part_number_detail.php?id=${row.id || row.pk}">View</a>
  ...
  document.getElementById('table').addEventListener('click', async (e) => {
    ...
    if (action === 'delete' && confirm('Delete this item?')) {
      const res = await fetch(`${API_BASE}/part-numbers/${id}/`, {
        method: 'DELETE',
        headers: authHeaders()
      });
    ...
    if (action === 'edit') {
      location.href = './part_number_form.php?id=' + id;
    }
  });
  ...
  createForm?.addEventListener('submit', async (e) => {
    ...
    payload['details'] = document.getElementById('create_details').value;
    payload['customer_part_number'] = document.getElementById('create_customer_part_number').value;
    const res = await fetch(`${API_BASE}/part-numbers/`, {
      method: 'POST',
      headers: authHeaders(),
